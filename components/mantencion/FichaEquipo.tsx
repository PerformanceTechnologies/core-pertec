...
export default function FichaEquipo({
  ...
  return (
    <li
      className={`group border bg-superficie transition ${abierto ? "border-naranjo shadow-[0_8px_24px_rgba(200,82,23,.08)]" : "border-borde hover:border-tinta/15"}`}
    >
      <button
        type="button"
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 flex flex-wrap items-center gap-1.5 text-[11px] text-tinta/50">
            <span
              ...
              style={{
                background: ESTADO_EQUIPO_COLOR[equipo.estado].bg,
                color: ESTADO_EQUIPO_COLOR[equipo.estado].texto,
              }}
            >
            ...
            {ultima && (
              <span>
                · última{" "}
                {new Date(ultima.iniciado_en).toLocaleDateString("es-CL", { day: "2-digit", month: "short" })}
              </span>
            )}
          </p>
        </div>
      ...
      {abierto && (
        <div className="flex flex-col gap-2.5 border-t border-borde bg-gradient-to-b from-crema/40 to-white py-3 pl-11 pr-3.5">
          {equipo.descripcion && (
            <p className="text-[13px] font-light leading-relaxed text-tinta/55">{equipo.descripcion}</p>
          )}

          {puedeOperar && (
            <div className="flex flex-wrap items-center gap-1.5 border border-borde bg-crema/60 px-3 py-2">
              <span className="mr-1 text-[11px] font-semibold uppercase tracking-[.1em] text-tinta/50">
                Estado:
              </span>
            ...
            {puedeOperar && (
              <>
                <button
                  type="button"
                  onClick={onEditar}
                  className="rounded-lg p-1.5 text-tinta/40 hover:bg-crema hover:text-naranjo"
                  title="Editar"
                >
                  ✎
                </button>
                <button
                  type="button"
                  onClick={onEliminar}
                  className="rounded-lg p-1.5 text-tinta/40 hover:bg-crema hover:text-red-600"
                  title="Eliminar"
                >
                  ×
                </button>
          ...
          {runs.length === 0 ? (
            <p className="border border-dashed border-borde bg-crema/50 py-4 text-center text-xs italic text-tinta/40">
