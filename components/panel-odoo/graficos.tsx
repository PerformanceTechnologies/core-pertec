...
export function GraficoAreaSimple({ datos, dataKey = "monto" }: { datos: Record<string, number | string>[]; dataKey?: string }) {
  ...
  if (datos.length < 2) return <PocoHistorial />;
  return (
    <div className={`${ALTO_GRAFICO} w-full`}>
      <ResponsiveContainer>
    ...
    <div className={`${ALTO_GRAFICO} w-full`}>
      <ResponsiveContainer>
        <BarChart data={datos} margin={{ top: 4, right: 4, left: 4, bottom: 0 }}>
          ...
          <Bar dataKey="ingreso" fill={TEAL} radius={[3, 3, 0, 0]} maxBarSize={28} />
          <Bar dataKey="gasto" fill={NARANJO} radius={[3, 3, 0, 0]} maxBarSize={28} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
...
export function GraficoDona({ datos }: { datos: { etapa: string; cantidad: number }[] }) {
  ...
  return (
    <div className={`${ALTO_GRAFICO} w-full`}>
      <ResponsiveContainer>
        <PieChart>
          <Pie data={datos} dataKey="cantidad" nameKey="etapa" innerRadius={26} outerRadius={44} paddingAngle={2}>
            {datos.map((_, i) => (
...
function SinDatos() {
  return (
    <div className={`flex ${ALTO_GRAFICO} w-full items-center justify-center text-xs text-tinta/40`}>
      Sin datos suficientes todavía.
    </div>
  );
}

function PocoHistorial() {
  return (
    <div className={`flex ${ALTO_GRAFICO} w-full items-center justify-center text-xs text-tinta/40`}>
