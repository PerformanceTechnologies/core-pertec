...
import { paginasComoImagen } from "@/lib/rendidor/pdf-a-imagen";
      ...
      await Promise.all(
        rendicion.gastos
          ...
          .map(async (g): Promise<RespaldoParaExcel | null> => {
            ...
            // Un PDF se pasa a imagen (cada página) antes de achicarlo: Excel no
            // puede mostrar un PDF embebido.
            const esPdf = respaldo.mimeType === "application/pdf";
            const pdf = esPdf ? await paginasComoImagen(respaldo.contenido) : null;
            const paginas = pdf
              ? (await Promise.all(pdf.pngs.map((png) => miniaturaParaExcel(png, "image/png")))).filter(
            ...
            return {
              gastoId: g.id,
              paginasAdicionales: paginas.slice(1),
              paginasPdf: pdf?.paginas,
