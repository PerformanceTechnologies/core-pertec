...
export interface FilaTarea {
  ...
  prioridad: string | null;
  fecha_ultimo_cambio_etapa: string | null;
  fecha_asignacion: string | null;
  horas_asignadas: number;
  horas_gastadas: number;
  avance: number;
  padre_odoo_id: number | null;
  subtareas: number;
  etiquetas: string | null;
  cliente: string | null;
}
...
export interface FilaProyecto {
  ...
  fecha_estado_salud: string | null;
  presupuesto: number;
  gastado: number;
  disponible: number;
...
const COLUMNAS_TAREA =
  ...
  "asignados, completado, color_hex, gastos_total, gastos_cantidad, prioridad, fecha_ultimo_cambio_etapa, " +
  "fecha_asignacion, horas_asignadas, horas_gastadas, avance, padre_odoo_id, subtareas, etiquetas, cliente";

const COLUMNAS_PROYECTO =
  "odoo_id, nombre, partner_nombre, responsable, fecha_inicio, fecha_vencimiento, etapa, estado_salud, " +
  "fecha_estado_salud, presupuesto, gastado, disponible, porcentaje_gastado, objetivos_total, " +
  "objetivos_hechos, gastos_cantidad, gastos_por_categoria, color_hex, supabase_id";
...
/**
 * Los proyectos activos y TODAS sus tareas, en una sola lectura.
 *
 * Igual que el CRM upstream: la tarjeta pide todo una vez y el detalle filtra en
 * el cliente. Sin las tareas cerradas no hay avance que mostrar, y sin los
 * proyectos completos no hay presupuesto ni salud -- que es justamente lo que
 * antes se quedaba en Odoo.
 */
export async function listarProyectosConTareas(): Promise<{ proyectos: FilaProyecto[]; tareas: FilaTarea[] }> {
  ...
  const proyectos = ((proyectosCrudos ?? []) as unknown as FilaProyecto[])
    ...
    .map((p) => ({ ...p, gastos_por_categoria: p.gastos_por_categoria ?? [] }));

  // Las tareas de un proyecto que el core dio por terminado no se listan: si no,
  // el detalle mostraria tareas de un proyecto que ya no aparece arriba.
  const vigentes = new Set(proyectos.map((p) => p.odoo_id));
  const tareas = ((tareasCrudas ?? []) as unknown as FilaTarea[]).filter(
    (t) => t.proyecto_odoo_id === null || vigentes.has(t.proyecto_odoo_id),
  );
